...
function getPlayerState(game, playerId) {
    ...
    return {
        ...
        mySunkShips: (_a = opponent === null || opponent === void 0 ? void 0 : opponent.sunkShips) !== null && _a !== void 0 ? _a : 0, // How many of the opponent's ships are sunk
...
app.prepare().then(() => {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('joinGame', (gameId) => {
            ...
            if (!game.players[socket.id]) {
                ...
                game.players[socket.id] = {
                    ...
                    ships: shipsConfig.map(s => (Object.assign(Object.assign({}, s), { positions: [], hits: 0 }))),
                    shots: new Set(),
                    sunkShips: 0,
                };
            ...
            if (game.playerOrder.length === 2 && game.status === 'waiting') {
                ...
                for (const playerId of game.playerOrder) {
                    ...
                    player.ships = shipsConfig.map(s => (Object.assign(Object.assign({}, s), { positions: [], hits: 0 })));
        ...
        socket.on('fire', ({ gameId, row, col }) => {
            console.log(`Received fire event from ${socket.id} for game ${gameId} at (${row}, ${col})`);
            const game = games[gameId];
            const player = game === null || game === void 0 ? void 0 : game.players[socket.id];
            if (!game || !player || game.status !== 'playing') {
                console.log(`Invalid fire event: Game not found (${!!game}), player not found (${!!player}), or game not playing. Game status: ${game === null || game === void 0 ? void 0 : game.status}`);
            ...
            if (targetCell === 1) { // Hit!
                ...
                const hitShip = opponent.ships.find(ship => ship.positions.some(pos => pos[0] === row && pos[1] === col));
                if (hitShip) {
                    hitShip.hits++;
        ...
        socket.on('newGame', ({ gameId }) => {
            ...
            if (game && game.status === 'finished') {
                ...
                console.log(`Player ${socket.id} attempted invalid 'newGame' request for game ${gameId}. Game status: ${game === null || game === void 0 ? void 0 : game.status}`);
            ...
            if (gameId) {
                ...
                if (game.status === 'playing') {
                    ...
                    if (remainingPlayerId && game.players[remainingPlayerId]) {
                        ...
                        io.to(remainingPlayerId).emit('update', Object.assign(Object.assign({}, finalState), { message: 'Opponent disconnected. You win!', status: 'finished', winner: remainingPlayerId // Ensure winner is explicitly set
                         }));
